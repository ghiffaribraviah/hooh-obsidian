...
#### Jawaban Soal 1
Pertama, identifikasi nilai $x_1$ dan $x_2$ untuk skenario yang diberikan:
- Untuk perempuan, $x_1 = 0$.
- Untuk dosis vaksin 2 mililiter, $x_2 = 2$.

Substitusikan nilai $x_1$ dan $x_2$ ke dalam model dugaan:
$$
...
Kemudian, ubah dari log-odds ke odds:
$$
...
#### Jawaban Soal 2A
Jika $Y_i$ mengikuti distribusi Poisson dengan parameter $\mu_i$, maka fungsi massa peluangnya (FMP) adalah:
$$
...
Subtitusikan ${\log(\mu_i)}$ dan ${\mu_i}$ menjadi:
$$
...
#### Jawaban Soal 2B
Identifikasi nilai peubah penjelas ($x_{1i}$ dan $x_{2i}$):
- Untuk pemegang polis perempuan, ${x_{1i} = 0}$.
- Untuk pola hidup sehat, ${x_{2i} = 1}$.

Substitusikan nilai-nilai tersebut ke dalam model dugaan:
$$
...
Estimasi frekuensi klaim untuk pemegang polis perempuan yang melakukan pola hidup sehat adalah sekitar 8.41. Artinya, seorang perempuan dengan pola hidup sehat diperkirakan akan membuat klaim sebanyak 8.41 kali.
...
#### Jawaban Soal 4A
Fungsi massa peluang (Probability Mass Function - PMF) untuk $Y_i \sim Bernoulli(\pi_i)$ adalah:
$$
...
Substitusikan $\pi_i$ dan $1 - \pi_i$ dalam bentuk $\eta_i$:
$$
...
Ganti $\eta_i$ kembali dengan $\beta_0 + \beta_1x_i$:
$$
